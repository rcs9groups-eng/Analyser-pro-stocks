...
class InstitutionalTraderPro:
    ...
    def get_multiple_timeframe_data(_self, symbol):
        ...
        try:
            ...
            # Hourly data (1 month)
            hourly_data = stock.history(period="1mo", interval="1h")
    ...
    def identify_supply_demand_zones(self, df):
        ...
        recent_demand = [z for z in demand_zones if z[2] > df.index[-30]]
        recent_supply = [z for z in supply_zones if z[2] > df.index[-30]]
    ...
    def generate_institutional_signals(self, daily_df, weekly_df, hourly_df):
        ...
        if current_daily['Volume_Ratio'] > 1.5:
            if "BUY" in signals:
                score += 6
            ...
            with st.spinner("Running institutional-grade analysis..."):
                ...
                if all(data is not None for data in [daily_data, weekly_data, hourly_data]):
                    ...
                    if "BUY" in action or "SELL" in action:
                        ...
                        with col3:
                            risk_reward = (targets[2]-current_price)/(current_price-stop_loss) if "BUY" in action else (current_price-targets[2])/(stop_loss-current_price)
                    ...
                    if current_daily is not None:
                        ...
                        with status_cols[1]:
                            st.metric("RSI", f"{current_daily['RSI_14']:.1f}")
                        with status_cols[2]:
                            volume_status = "HIGH" if current_daily['Volume_Ratio'] > 1.5 else "NORMAL"
